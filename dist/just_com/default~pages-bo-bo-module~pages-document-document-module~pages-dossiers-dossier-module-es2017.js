...
let AddEditDocumentComponent = class AddEditDocumentComponent {
    constructor(dialogRef, data, translate, documentService, layoutService, formBuilder, configService, dialog) {
    ...
    getListNatureDoc() {
        this.documentService.getListNatureDoc().subscribe(response => {
            if (response['code'] === '200') {
                this.listeNatureDoc = response['payload'];
            }
        }, error => {
    ...
    getListTypeDoc() {
        this.documentService.getListTypeDoc().subscribe(response => {
            if (response['code'] === '200') {
                this.listeTypeDoc = response['payload'];
            }
        }, error => {
            console.log('error :: ' + error);
        });
    ...
    handleFileInput(files) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            this.listeFileAdded.push(Object.assign({}, this.fileToUpload, { fileName: this.fileToUpload.name, nodeRef: null }));
        }
        this.dataSource = new _angular_material_table__WEBPACK_IMPORTED_MODULE_5__["MatTableDataSource"](this.listeFileAdded);
    }
    removeFile(index) {
        debugger;
        const selectedFile = this.listeFileAdded[index];
        ...
        dialogConfig.data = {};
        const dialogRef = this.dialog.open(_delete_file_confirmation_delete_file_confirmation_component__WEBPACK_IMPORTED_MODULE_14__["DeleteFileConfirmationComponent"], dialogConfig);
        if (selectedFile.nodeRef) {
            dialogRef.afterClosed().subscribe(result => {
                if (result && result.status === 'OK') {
                    this.documentService.deleteFileWithNodeRef(selectedFile.nodeRef).subscribe(response => {
                        if (response['code'] === '200') {
                            this.getListFileAdded();
                            this.listeFileAdded = null;
                        }
                    });
                }
            });
            ...
            dialogRef.afterClosed().subscribe(result => {
                if (result && result.status === 'OK') {
                    this.listeFileAdded.splice(index, 1);
    ...
    DownloadFile(index) {
        const selectedFile = this.listeFileAdded[index];
        this.documentService.DowloadFile(selectedFile.nodeRef).subscribe(response => {
    ...
    getListFileAdded() {
        this.documentService.listDocAdded(this.documentService.affModifDoc ? this.data.docModif.id : null).subscribe(response => {
            if (response['code'] === '200') {
                this.listeFileAdded = response['payload'];
                console.log("eeeeeeeeeeeeeeetest", this.listeFileAdded);
                this.dataSource = new _angular_material_table__WEBPACK_IMPORTED_MODULE_5__["MatTableDataSource"](this.listeFileAdded);
    ...
    getListDocument() {
        this.documentService.getListDocumentWhitoutFilte().subscribe(response => {
            if (response['code'] === '200') {
                this.listeDoc = response['payload'];
            }
        }, error => {
            console.log('error :: ' + error);
        });
    ...
    Validatort(form) {
        const condition = this.formDocument.get('nomFichier').value.toString() == 'test';
        return condition ? { passwordsDoNotMatch: true } : null;
    }
    isNameDocExist(value) {
        // debugger;
        this.codeExist = false;
        if (value !== undefined && value != null) {
            this.codeExist = this.listeDoc.find(ident => ident.nomFichier == value);
    ...
    addDoc() {
        ...
        if (this.formDocument.valid) {
            ...
            let dataTosend;
            let idCourrArr = this.data.idSrc;
            let idDossier = this.data.idSrc;
            ...
            // this.submit= true;
            const filesToUpload = this.listeFileAdded ? this.listeFileAdded.filter(element => !element.nodeRef) : [];
            if (filesToUpload && filesToUpload.length) {
                const listFiles = filesToUpload.map(element => {
                    return this.filesUploaded.find(file => file.name === element.fileName);
                });
                this.documentService.addDocument(listFiles, objectToSend).subscribe(response => {
                    if (response['code'] === '200') {
                        dataTosend = this.data;
                        this.documentService.affModifDoc === false;
                        //this.submit=true ;
                        this.documentService.selectDoc(dataTosend);
                        ...
                        debugger;
                        this.afficheControlName = true;
                    }
                });
            }
            else {
                this.documentService.addDocument(null, objectToSend).subscribe(response => {
                    if (response['code'] === '200') {
                        dataTosend = this.data;
                        this.documentService.affModifDoc === false;
                        //this.submit=true ;
    ...
    controlName(name) {
        if (this.formDocument.get('nomFichier').value == name) {
            ...
            this.nameVerif = this.listeDoc.find(ident => ident.nomFichier == name);
        }
    ...
    editDoc() {
        ...
        if (this.formDocument.valid) {
            ...
            if (filesToUpload && filesToUpload.length) {
                ...
                this.documentService.updateDocument(listFiles, objectToSend).subscribe(response => {
                    dataTosend = this.data;
                    this.documentService.affModifDoc === false;
                    this.submit = true;
                    this.close({ status: 'OK' });
                ...
                this.documentService.updateDocument(null, objectToSend).subscribe(response => {
                    if (response['code'] === '200') {
                        dataTosend = this.data;
                        this.documentService.affModifDoc === false;
                        //this.submit=true ;
...
AddEditDocumentComponent.ctorParameters = () => [
...
let DeleteFileConfirmationComponent = class DeleteFileConfirmationComponent {
...
DeleteFileConfirmationComponent.ctorParameters = () => [
...
let DocumentRoutingModule = class DocumentRoutingModule {
};
...
let DocumentComponent = class DocumentComponent {
    constructor(documentService, searchObjectTreatment, configService, translate, dialog) {
        this.documentService = documentService;
    ...
    initDataListDocument() {
        debugger;
        this.documentService.getListDocumentWithFiltre(this.idSrc, this.typSrc, this.searchObject).subscribe(response => {
    ...
    getListDocument() {
        this.documentService.getListDocumentWhitoutFilte().subscribe(response => {
            if (response['code'] === '200') {
                this.listeDoc = response['payload'];
                this.dataSource = new _angular_material_table__WEBPACK_IMPORTED_MODULE_4__["MatTableDataSource"](this.listeDoc);
                console.log("listeeeentite", this.listeDoc);
            }
        }, error => {
            console.log('error :: ' + error);
        });
    ...
    getDocById(id) {
        this.documentService.getListDocumentById(id).subscribe(response => {
            if (response['code'] === '200') {
                this.documentById = response['payload'];
            }
        }, err => {
            console.log(err);
        });
    ...
    onAddDocument() {
        ...
        const dialogRef = this.dialog.open(_add_edit_document_add_edit_document_component__WEBPACK_IMPORTED_MODULE_9__["AddEditDocumentComponent"], dialogConfig);
        dialogRef.afterClosed().subscribe(result => {
            if (result && result.status === 'OK') {
                this.searchObject = this.searchObjectTreatment.treatementSearchObject(5, 0, "dtMaj", "desc nulls last");
                this.initDataListDocument();
    ...
    onEditDocument() {
        ...
        dialogConfig.hasBackdrop = true;
        //docModif = {};
        this.currentlistDoc = Object.assign({}, this.selectedDocument);
...
let DocumentService = class DocumentService {
    ...
    getListDocumentWithFiltre(idSrc, typeSrc, docForm) {
        debugger;
    ...
    updateDocument(fileToUpload, docForm) {
        debugger;
    ...
    addDocument(fileToUpload, docForm) {
        debugger;
        const formData = new FormData();
        if (fileToUpload) {
            for (let i = 0; i < fileToUpload.length; i++) {
    ...
    getfileToDownloaByParam(CodeEtat, doc) {
...
//# sourceMappingURL=default~pages-bo-bo-module~pages-document-document-module~pages-dossiers-dossier-module-es2017.js.map