...
var LoginPageComponent = /** @class */ (function () {
    ...
    function LoginPageComponent(route, renderer, toastr, router, accountService, translate, layoutService, configService) {
        this.route = route;
    ...
    LoginPageComponent.prototype.ngAfterViewInit = function () {
        var _this = this;
        if (this.config.layout.dir) {
            setTimeout(function () {
                _this.onChangeLanguage(_this.currentLang);
            }, 0);
        }
    };
    ...
    LoginPageComponent.prototype.login = function () {
        ...
        this.accountService.login(this.f.username.value, this.f.password.value).subscribe(function (response) {
            if (response['code'] === '200') {
                sessionStorage.setItem('token', response['payload']['token']);
                sessionStorage.setItem('lang', _this.currentLang);
                //this.whoami();
                _this.router.navigate(['/home']);
            }
            else if (response['code'] === '201') {
                _this.toastr.error('', 'Pas d\'access pour ce compte');
            }
            else {
                _this.toastr.error('', 'Identifiant ou mot de passe incorrect.');
            }
        }, function (error) {
            _this.toastr.error('', 'Erreur interne système');
        });
        this.loginForm.reset();
    };
    LoginPageComponent.prototype.whoami = function () {
        var _this = this;
        this.accountService.whoiam()
            .subscribe(function (response) {
            console.log('response ::', response);
            _this.userInfo = response['payload'];
        }, function (error) {
        });
    };
    ...
    LoginPageComponent.prototype.sendOptions = function (dir) {
        this.layoutService.emitChange({ direction: dir });
        this.configService.setDirection(dir);
        sessionStorage.setItem('direction', dir);
    };
    LoginPageComponent.prototype.emitDirection = function (lang) {
        lang === 'ar' ? this.sendOptions('rtl') : this.sendOptions('ltr');
    };
    LoginPageComponent.prototype.onChangeLanguage = function (lang) {
...
//# sourceMappingURL=pages-authentification-authentification-module-es5.js.map